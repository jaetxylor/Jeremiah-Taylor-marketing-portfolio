...
function renderBuilt(){
  ...
  el.innerHTML=D.websitesBuilt.map((x,i)=>`<article class="web-card reveal" data-delay="${i*50}">
    ...
    <div class="web-card-copy"><div class="web-meta"><span>${esc(x.type)}</span><span>0${i+1}</span></div><h3>${esc(x.name)}</h3><p>${esc(x.description)}</p><div class="tags">${x.tools.map(t=>`<span>${esc(t)}</span>`).join('')}</div><a class="card-link" href="${esc(x.url)}" target="_blank" rel="noopener">Visit live site ↗</a></div>
  </article>`).join('');
}

function renderManaged(){
  document.getElementById('web-managed-grid').innerHTML=D.websitesManaged.map(x=>`<article class="managed-card reveal"><span class="platform">${esc(x.platform)}</span><h4>${esc(x.name)}</h4><p>${esc(x.description)}</p>${x.url?`<a href="${esc(x.url)}" target="_blank" rel="noopener">Open site ↗</a>`:`<a aria-label="Confidential work">NDA protected</a>`}</article>`).join('');
}

function renderCampaigns(){
  document.getElementById('campaign-grid').innerHTML=D.publicCampaigns.map(x=>`<article class="campaign-card reveal"><span class="campaign-platform">${esc(x.platform)}</span><h4>${esc(x.name)}</h4><p>${esc(x.scope)}</p></article>`).join('');
}
